...
class RowBaseModel(BaseModel):
    ...
    def update(self, other, validate=False):
        NotImplementedError(
            f"update(other) has not been implemented for {self.__class__}"
        )

    def title_name(self):
        NotImplementedError(
            f"title_name() has not been implemented for {self.__class__}"
        )
...
def transfer_items(items: List[CachedStockItem], location: StockLocation):
    ...
    StockItem.adjustStockItems(
        api, method="transfer", items=_items, location=location.pk
    )
    ...
    try:
        ...
        if e.response is not None:
            ...
            try:
                ...
                raise ApiException("failed to decode body")
    ...
    def update(self, other, validate=False, allow_greater=False):
        if validate:
            ...
            if not allow_greater and other.quantity > oq:
                raise ValueError(
                    f"Quantity is greater than the original stock quantity ({oq})"
                )
