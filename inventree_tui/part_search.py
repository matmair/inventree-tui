...
class PartSearchTree(Widget):
    ...
    def add_part(self, cached_part: CachedPart, expand=False):
        node = self.part_tree.root.add(
            cached_part.part.name, data=cached_part, allow_expand=False, expand=False
        )
        if expand:
    ...
    def add_stock_item(self, node, stock_item: CachedStockItem):
        node.add(
            f"Stock #{stock_item.item.pk}, location: {stock_item.location.name}, Q: {stock_item.item.quantity}",
            data=stock_item,
            allow_expand=False,
            expand=False,
        )
